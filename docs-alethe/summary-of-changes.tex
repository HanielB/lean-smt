...
\begin{table}[t]
  ...
  \begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}Xr@{}}
    ...
    Part & Files & Lines \\
    \midrule
    \alethe parser & \file{Syntax}, \file{Parser}, \file{Realize} (3 new) & 952 \\
    ...
    Generated RARE table & \file{Rare/Rules.lean} (1 new; generator \file{gen.py} 445, input \file{rewrites.eo} 513) & 1\,379 \\
    ...
    Changes to pre-existing library files & 13 modified & $+1\,389$ / $-791$ \\
    ...
    Scripts & \file{check-alethe.sh}, \file{setup-alethe.sh}, \file{rule-boxplots.py} (3 new) & 657 \\
    Report and notes & \file{docs-alethe/}: \TeX{} 2\,071, Markdown 606, plus figures and CSVs & \\
    \bottomrule
  \end{tabularx}
  \caption{Size of each part of the branch. Lines are lines of \lean unless
    stated otherwise. The 124 commits are on top of \code{76b4eea}.}
  \label{tab:sizes}
\end{table}
...
\item[\file{Prop/ResNorm.lean} (397 lines, 18 theorems)] A checker for
  ...
  \code{smt.alethe.reflect}, default off in the library and set by the
  evaluation's runner; the \cvcv path does not use it yet. It carries
  \code{resolution} and the \code{contraction}/\code{reordering} steps that
  really permute or deduplicate. It does \emph{not} carry \code{or} and
  \code{weakening}, nor a \code{contraction} whose premise is already its
  conclusion: those paths do essentially no work already --- for \code{or} the
  premise's proposition \emph{is} the stated clause --- and reifying two
  clauses for them was measured 3 to 8 times slower.
\end{description}
...
\begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}X@{}}
  ...
  \code{contraction} & new lemma \code{orN_of_map}; \code{ResNorm} when set, unless \\
  & the premise is already the conclusion \\
  \code{or} & new lemma \code{orN_of_map} \\
