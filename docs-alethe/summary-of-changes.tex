...
\begin{table}[t]
  ...
  \begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}Xr@{}}
    ...
    Rule reconstruction & \file{Basic}, \file{Clause}, \file{Prop}, \file{Arith}, \file{UF}, \file{Quant}, \file{Polyeq}, \file{Lemmas}, \file{Rare}, \file{Rare/Basic} (10 new) & 3\,187 \\
...
\code{bounded_farkas} (rewritten by \code{local} into \code{la_generic}) and
\code{reordering}. The non-linear multiplication rules are left out as well.
The handlers also accept those rules, so that a raw solver proof can be
checked, but they are not covered here.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[node distance=3mm and 3mm, rc/.style={old, text width=72mm}]
    ...
    \node[new, left=14mm of proplem] (prop) {\file{Prop.lean}\\{\scriptsize 46 propositional and CNF rules}};
    \node[new, left=14mm of arithlem] (arith) {\file{Arith.lean}\\{\scriptsize la\_generic, la\_tautology, poly\_simp*,}\\{\scriptsize div\_intro, la\_disequality, \ldots}};
...
\subsection{Clausal rules (\file{Clause.lean}, 210 lines)}
...
\begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}X@{}}
  ...
  Rule & Component \\
  \midrule
  \code{resolution}, \code{th_resolution}, \code{strict_resolution} & existing \code{Prop.reconstructResolution}, \code{Prop.getResolutionResult}; \\
...
\subsection{Propositional rules (\file{Prop.lean}, 296 lines)}

Of the 46 rules, 40 are direct applications of lemmas that existed in
...
\begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}X@{}}
  ...
  \code{bfun_elim} & new meta code, a classical case split (\code{proveByCases}) \\
  \bottomrule
\end{tabularx}
\end{center}
...
The Farkas driver behind \code{la_generic} and \code{la_tautology} is new
meta code, about 340 lines: the negations of the clause's literals are
...
\begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}X@{}}
  ...
  Rule & Component \\
  \midrule
  \code{la_generic}, \code{la_tautology} & existing \code{Int.sum_ub}$_{1..9}$, \code{Rat.sum_ub}$_{1..9}$; existing \code{mul_pos_lt/le/eq}, \code{mul_neg_eq} (Int, Rat); existing \code{Int.polyNorm}, \code{Rat.polyNorm} and their native variants; existing \code{Prop.notNotElim}; \lean core \code{omega} for integer tightening; new lemmas \code{farkas_lt/le/eq} (Int, Rat), \code{orN_of_impliesN_not} \\
...
\subsection{Equality and rewriting rules (\file{UF.lean}, 550 lines)}
