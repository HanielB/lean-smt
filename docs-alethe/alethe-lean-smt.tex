...
premises as sub-rewrites and mirrors those two routes; a premise-free instance
keeps its strict structural reading. Of the affected proofs --- 175 QF\_UFIDL
(\texttt{pete2}, \texttt{pete}, \texttt{uclid}) and 24 QF\_LIA --- 198 now
check: 189 \emph{valid} and 9 \emph{holey}, the latter QF\_LIA
\texttt{calypto} proofs holey only for their \rulename{lia\_generic} steps.
The step is decomposed into \rulename{cong}, \rulename{trans} and
\rulename{semilattice\_simp}, which the checker already reconstructs, so no
lean-smt change was needed. The single proof still rejected fails on a
\emph{premise-free} \rulename{ac\_simp} whose conclusion \verit left
under-flattened --- \texttt{(and (and A B) C)} eight levels down, where the
normal form is \texttt{(and A B C)}. The equality holds, so reading the rule
up to a common normal form would accept it, but that is the relaxation the
premise-free case deliberately does not make. The identifiable remainder is 24
