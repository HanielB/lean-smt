...
small clausal rules --- 9\,300$\times$ on \rulename{or\_neg} and
30\,000$\times$ on \rulename{and} --- where \carcara compares two
...
checker's time, and \rulename{and} a further 14\%. Half of the number is one
rule, and no amount of work on the arithmetic or rewriting rules moves it: the
remedy for the timeouts is a reflective resolution checker.

\rulename{and}, though, is not of that kind, and the figure was what made us
look. Its median is 8.6\,ms, \emph{above} \rulename{resolution}'s 5.2\,ms and
higher than every rule in the figure but \rulename{poly\_simp} --- the highest
per-step cost of any high-volume rule, on a rule whose whole content is to name
the $i$-th conjunct of a premise. That was an artefact of how the proof term was
built rather than anything the rule demands. The reconstruction sends an n-ary
...
it; do not manufacture a list and index it.

Third, the structural AC rules that replaced \rulename{aci\_simp}
(Section~\ref{sec:carcara}) are cheap on both sides:
\rulename{semilattice\_simp} at a median of 4.3\,ms and \rulename{poly\_simp}
at 11\,ms for the checker, with ratios of 833$\times$ and 781$\times$, better
