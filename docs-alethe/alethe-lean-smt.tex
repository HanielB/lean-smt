...
and Section~\ref{sec:shapes} the comparison of the kernel submission shapes.
...
\paragraph{Outcomes.} Table~\ref{tab:smtlib} gives the round per logic.
\cvcv proves 20\,037 benchmarks unsatisfiable and \verit 19\,196; \carcara
accepts 19\,194 and 17\,823 of the proofs, and the checker validates 16\,853
and 15\,334. Of the proofs the checker does not validate, 2\,287 and 2\,494
are timeouts, 5 and 64 exhaust its memory, and 2 and 54 have a trusted step.
The columns are independent verdicts on the same benchmark, not a partition:
the checker validates more QF\_UFLIA \verit proofs than \carcara accepts,
because \carcara reports a proof with a \rulename{lia\_generic} step holey
while the checker closes that step with \texttt{omega}. The picture per logic
is uneven in an informative way: the quantified logics and the arithmetic ones
are nearly complete (99\% of UF, UFLIA, QF\_LIA), while QF\_UF, whose proofs
are an order of magnitude larger, is at 59\%, and QF\_IDL, whose
bounded-model-checking proofs are large too, at 58\% and 41\%.

\paragraph{Why \carcara does not check every proof.} The gap between
\emph{unsat} and \emph{\carcara} in Table~\ref{tab:smtlib} --- 843 benchmarks
for \cvcv and 1\,373 for \verit --- is almost entirely outside \carcara. For
\cvcv, 842 of the 843 are benchmarks for which \emph{no proof exists to
check}: the solver answered unsatisfiable within its 120\,s internal limit but
was killed by the 150\,s external one while printing the proof, leaving an
empty file. Only one is an elaboration error. For \verit the 1\,373 split five
ways: 653 are \carcara \emph{parser} errors, all of the form ``expected
\texttt{Real}, got \texttt{Int}'' on the proof text even under
\texttt{-{}-allow-int-real-subtyping}, concentrated in a few families (409 in
QF\_UFLRA's \texttt{mathsat}, 95 and 85 in QF\_LRA's \texttt{meti-tarski} and
\texttt{LassoRanker}, 47 in QF\_RDL's \texttt{sal}); 226 are steps \carcara's
own checker rejects, of which the identifiable ones are 24
\rulename{and\_simplify} and 13 \rulename{ac\_simp}; 252 are holey, all
\verit's oracle rule \rulename{lia\_generic}, which carries no certificate and
which \carcara does not check either; 151 exceed \carcara's own 300\,s limit,
on proofs of 130 to 230\,MB; 62 exhaust the 12\,GB during the check or the
elaboration; and 29 have no proof, \verit having been killed at 150\,s. So of
the whole gap only the 653 parser errors and the 226 rejected steps are
\carcara's to close, and the parser errors are a single defect in the
integer/real subtyping of \verit's proof text.

\begin{table}[p]
  \centering
  \caption{The SMT-LIB round, 23\,328 benchmarks per solver. \emph{unsat} is
    what the solver proved; \carcara the proofs it accepts; then the checker's
    outcomes --- proofs validated with no trusted step, timeouts, runs that
    exhausted its 10\,GB, and proofs with a trusted step (\emph{holey}). The
    columns are independent verdicts on the same benchmark, so the checker's
    do not sum to \carcara's. The last column is the median factor by which
    the checker, elaboration included, is slower than \carcara on the
    benchmarks \emph{both} validate.}
  \label{tab:smtlib}
  \small
  ...
  \begin{tabular}{l l r r r @{\quad} r r r r @{\quad} r}
    ...
    \cmidrule(lr){6-9}
    solver & logic & bench. & unsat & \carcara & valid & t/o & mem & holey & slower \\
    \midrule
    \multirow{13}{*}{\cvcv}
      & QF\_UF    & 4\,361 & 4\,324 & 4\,296 & 2\,527 & 1\,763 & 3 & 0 & 135$\times$ \\
      & QF\_IDL   &    917 &    633 &    475 &    276 &    196 & 0 & 2 &  75$\times$ \\
      & QF\_RDL   &    113 &     88 &     74 &     39 &     35 & 0 & 0 & 120$\times$ \\
      & QF\_LIA   & 4\,748 & 2\,794 & 2\,511 & 2\,341 &    152 & 0 & 0 & 221$\times$ \\
      & QF\_LRA   &    703 &    582 &    526 &    425 &    101 & 0 & 0 &  86$\times$ \\
      & QF\_UFIDL &    422 &    268 &     38 &     30 &      8 & 0 & 0 &   2$\times$ \\
      & QF\_UFLIA &    192 &    174 &    172 &    166 &      4 & 0 & 0 & 222$\times$ \\
      & QF\_UFLRA &    511 &    511 &    441 &    432 &      9 & 0 & 0 &  26$\times$ \\
      & UF        & 3\,505 & 3\,155 & 3\,155 & 3\,145 &      7 & 2 & 0 & 369$\times$ \\
      & UFIDL     &     53 &     53 &     53 &     53 &      0 & 0 & 0 & 717$\times$ \\
      & UFLIA     & 7\,793 & 7\,445 & 7\,443 & 7\,409 &     12 & 0 & 0 & 210$\times$ \\
      & UFLRA     &     10 &     10 &     10 &     10 &      0 & 0 & 0 & --- \\
    \cmidrule(l){2-10}
      & total     & 23\,328 & 20\,037 & 19\,194 & 16\,853 & 2\,287 & 5 & 2 & 213$\times$ \\
    \midrule
    \multirow{13}{*}{\verit}
      & QF\_UF    & 4\,361 & 4\,193 & 4\,175 & 2\,446 & 1\,729 &  0 &  0 &  506$\times$ \\
      & QF\_IDL   &    917 &    645 &    538 &    222 &    293 & 14 &  0 &   52$\times$ \\
      & QF\_RDL   &    113 &     95 &     28 &     15 &     13 &  0 &  0 & 3\,699$\times$ \\
      & QF\_LIA   & 4\,748 & 2\,611 & 2\,314 & 2\,220 &    187 & 17 & 10 &  698$\times$ \\
      & QF\_LRA   &    703 &    580 &    387 &    282 &    105 &  0 &  0 &  167$\times$ \\
      & QF\_UFIDL &    422 &    271 &     64 &     47 &     16 &  0 &  0 &  499$\times$ \\
      ...
      & UFLRA     &     10 &     10 &     10 &     10 &      0 &  0 &  0 & --- \\
    \cmidrule(l){2-10}
      & total     & 23\,328 & 19\,196 & 17\,823 & 15\,334 & 2\,494 & 64 & 54 & 497$\times$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{l r r @{\quad} r r r r @{\quad} r r @{\quad} r}
    ...
    & & & \multicolumn{4}{c}{\tactic} & & & \\
    \cmidrule(lr){4-7}
    solver & unsat & \carcara & valid & t/o & mem & holey & \carcara h & \tactic h & slower \\
    \midrule
...
and QF\_LRA, 9\,812 benchmarks per solver) and on the Sledgehammer round; and
the comparison with the \texttt{native} variant, made on the twelve-logic
round.
...
\paragraph{Native evaluation.} The twelve-logic round ran the per-step shape
twice, plain and with \texttt{native}, which evaluates the reflective
normalizers and the decision procedures through compiled code instead of the
kernel and so puts Lean's compiler in the trusted base. Native validates 23
more \cvcv proofs (16\,876 against 16\,853) and 9 more \verit ones (15\,343
against 15\,334), with the same trusted steps; paired per proof, 15\,333
\verit proofs are validated by both, 10 by native only and one by the
per-step arm only, and native is faster on nearly every proof both validate.
The gain is real but small, because the kernel's time is dominated by the
resolution chains rather than by the normalizers that \texttt{native}
accelerates; it is the wrong place to spend trust for what it buys.
